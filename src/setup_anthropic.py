...
if _base_url:
    client = anthropic.Anthropic(api_key="dummy", base_url=_base_url)
else:
...
_FEW_SHOT_EXAMPLES_JSON = [
    (json.dumps(ex_snap, indent=2), json.dumps(ex_ann, indent=2))
    for ex_snap, ex_ann in FEW_SHOT_EXAMPLES
]


def build_few_shot_prompt(clean: dict) -> str:
    lines = [PROMPT_HEADER, "Here are examples of correctly diagnosed snapshots:"]

    for i, (ex_snap_json, ex_ann_json) in enumerate(_FEW_SHOT_EXAMPLES_JSON, 1):
        lines.append(f"\n--- Example {i} ---")
...
def build_prompt(snapshot: dict, strategy: str = "zero_shot") -> str:
    ...
    if strategy == "few_shot":
        return build_few_shot_prompt(clean)
...
def rule_based_diagnose(snapshot: dict) -> dict:
    ...
    data_staged        = exe.get("data_staged", True)
...
def diagnose(snapshot: dict, model: str = "claude-haiku-4-5", strategy: str = "zero_shot") -> dict:
    ...
    # classify_only: enforce strict schema so the model emits only the bottleneck field.
    # Other strategies rely on prompt instructions to return the full 4-field JSON object.
    if strategy == "classify_only":
        output_config = {
            "format": {
                "type": "json_schema",
...
def evaluate(snapshots: list[dict], model: str = "claude-haiku-4-5", strategy: str = "zero_shot") -> dict:
    results = []
    ...
    mem_peak_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024
